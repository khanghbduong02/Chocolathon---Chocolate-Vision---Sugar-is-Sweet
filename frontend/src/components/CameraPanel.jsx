...
const CameraPanel = forwardRef(function CameraPanel(
  { phase, micOn, boxLocked, capturedImage, annotatedImage, onCameraReady },
  ref,
) {
    ...
    async (requestedFacingMode = facingMode) => {
      ...
      for (const constraints of deviceConstraints) {
        try {
          ...
          if (!hasUsableLight(videoRef.current))
            throw new Error("Camera produced a blank or dark frame.");
          setCameraReady(true);
          onCameraReady?.(true);
          return;
        } catch (error) {
      ...
      onCameraReady?.(false);
      setCameraError(
        lastError?.message || "No available camera produced a usable image.",
      );
    },
  ...
  return (
    <section className="camera-panel">
      <div className="panel-heading">
        ...
        <span className={`camera-status ${cameraReady ? "ready" : ""}`}>
      ...
      <div className="camera-frame">
        {phase === "review" && capturedImage ? (
          showClassifications && annotatedImage ? (
            <img
              ...
              alt="Captured box with classified items"
            />
          ) : (
            <img
              className="captured-frame"
              src={capturedImage}
              alt="Captured chocolate box"
            />
          )
        ) : (
          <video
        ...
        {phase === "scanning" && (
          <div className="camera-scan-overlay">Scanning box...</div>
        )}
      </div>
      {cameraError && <p className="inline-error">{cameraError}</p>}
      <div className="camera-actions">
        {phase === "review" && capturedImage && annotatedImage && (
          <button
            className="button secondary"
            onClick={() => setShowClassifications((visible) => !visible)}
          >
            {showClassifications
        ...
        {cameraReady && phase === "listening" && (
          <button className="button secondary" onClick={switchCamera}>
            Switch camera
          </button>
        )}
      </div>
      <p className="panel-hint">
        The captured frame is saved with the order as original evidence.
      </p>
      {boxLocked && (
        <p className="panel-hint">
          This box is saved. Start a new box to capture again.
        </p>
      )}
    </section>
  );
